...
def return_fetching_dates(base_currency, starting_date, connection, table, time_interval):
    ...
    result = query_db(text(f"""
                                SELECT CASE 
                                         WHEN MIN(date_recorded) = MAX(date_recorded) AND CAST(:time_interval AS TEXT) ILIKE '%month%'
                                  ...
                                  FROM (
                                         SELECT x.*, SUM(i) OVER(ORDER BY date_recorded) AS grp
                                           FROM (
                                                  SELECT t.*,
                                                         CASE WHEN date_recorded > LAG(date_recorded) OVER(order BY date_recorded) + CAST(:time_interval AS INTERVAL)
                                                                THEN 1 
                                                              ELSE 0 
                                                         END AS i
                                                       FROM (
                                                             SELECT DATE(dates_recorded) AS date_recorded
                                                               FROM generate_series(:starting_date, CURRENT_DATE, CAST(:time_interval AS INTERVAL)) AS dates_recorded
...
def main(connection):
    ...
    for i, currency in enumerate(valid_currencies):
        ...
        logging.info(f"{fetching_dates}") #debug
